...
const why = computed(() => {
  ...
  if (e.status === 'finding' && e.finding) {
    const lift = e.deviation !== null && e.deviation !== undefined ? formatValue(e.deviation, 'lift') : ''
...
<template>
  <nav class="bar" aria-label="Drill path">
    <button type="button" class="back" :disabled="!dashboard.steps.length" @click="dashboard.back()">
      ← Back
    </button>
    <ol class="crumbs">
      <li>
